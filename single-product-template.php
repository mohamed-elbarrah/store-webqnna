...
$share_value = get_post_meta($product_id, 'share_value', true);
...
$related = do_shortcode('[donates orderby="rand" not_in="'.$product_id.'" two_in_row=1]');
...
$received_donations = qs_get_product_net_revenue($product_id);

if(!$project_goal || !is_numeric($project_goal))$project_goal = 0;
...
$s = file_get_contents(QS_DIR . '/html/single-product-template.html');
...
$s = str_replace('[percentage]', $percentage, $s);
$s = str_replace('[percentage_width]', $percentage . '%', $s);
$s = str_replace('[received_donations]', $received_donations, $s);
$s = str_replace('[project_goal]', $project_goal, $s);
